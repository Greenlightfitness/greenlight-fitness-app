...
const SessionBuilder: React.FC<SessionBuilderProps> = ({ planId, weekId, session, onClose, onSaveComplete }) => {
  const { t } = useLanguage();
  const [title, setTitle] = useState(session.title);
  const [description, setDescription] = useState(session.description || '');
  
  // Deep copy initial data to avoid reference mutation bugs
  ...
  const handleCreateNewBlock = () => {
    // Default name strategy: "Block [Letter]"
  ...
  const handleImportBlock = (blockData: any) => {
      // Deep copy block data to ensure new IDs
      const newBlock: WorkoutBlock = {
          ...blockData,
          id: generateId(),
  ...
  const confirmDeleteBlock = () => {
    ...
    const updated = workoutData.filter(b => b.id !== deleteData.blockId);
    setWorkoutData(updated);
    setDeleteData(null);
  };
  ...
  const handleExerciseEditorSave = () => {
  ...
  const confirmDeleteExercise = () => {
    ...
    setWorkoutData(workoutData.map(block => {
      ...
      const updatedExercises = block.exercises.filter(e => e.id !== deleteData.exerciseId);
  ...
  const updateExerciseNotes = (blockId: string, exerciseId: string, notes: string) => {
    setWorkoutData(workoutData.map(block => {
      ...
      return {
        ...block,
        exercises: block.exercises.map(ex => ex.id === exerciseId ? { ...ex, notes } : ex)
      };
    }));
  };
  ...
  return (
    <div className="fixed inset-0 z-[50] bg-[#121212] flex flex-col animate-in fade-in duration-200">
      ...
      <ConfirmationModal 
        isOpen={!!deleteData}
        title={deleteData?.type === 'block' ? "Delete Block?" : "Delete Exercise?"}
      ...
      <ExerciseEditorModal 
         ...
         onSave={handleExerciseEditorSave}
      />
            ...
            return (
                <div key={block.id} className="relative group/block-wrapper">
                    ...
                    >
                        ...
                        `}>
                            <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-[200px] flex-wrap">
                                <div className="flex flex-col gap-1 mr-2">
                                     <button 
                                        onClick={() => moveBlock(bIndex, bIndex - 1)}
                                        disabled={bIndex === 0}
                                     ...
                                     <button 
                                        onClick={() => moveBlock(bIndex, bIndex + 1)}
                                        disabled={bIndex === workoutData.length - 1}
                                        className="text-zinc-600 hover:text-white disabled:opacity-30"
                                     >
                                         <ChevronDown size={14} />
                                     </button>
                                ...
                                {isCircuit && (
                                    <div className="flex items-center gap-2 ml-2 border-l border-zinc-700 pl-4 animate-in fade-in slide-in-from-left-2 w-full sm:w-auto mt-2 sm:mt-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input 
                                                className="w-12 bg-zinc-950 border border-zinc-700 rounded px-2 py-1 text-center text-white focus:border-orange-500 outline-none"
                                                value={block.rounds || ''}
                                                onChange={(e) => updateBlockMeta(block.id, 'rounds', e.target.value)}
                                                placeholder="3"
                                            />
                                        </div>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input 
                                                className="w-16 bg-zinc-950 border border-zinc-700 rounded px-2 py-1 text-center text-white focus:border-orange-500 outline-none"
                                                value={block.restBetweenRounds || ''}
                                                onChange={(e) => updateBlockMeta(block.id, 'restBetweenRounds', e.target.value)}
                                                placeholder="90s"
                                            />
                                        </div>
                                    </div>
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto justify-end mt-2 sm:mt-0">
                                ...
                                >
                                    <Plus size={16} /> <span className="">{t('planner.addExercise')}</span>
                                </button>
                                <button 
                                    onClick={() => setDeleteData({ type: 'block', blockId: block.id })}
                        ...
                        {/* Exercises Container */}
                        <div className={`
                            divide-y divide-zinc-800 p-2
                            ${isSuperset ? 'border-l-4 border-[#00FF00]/50 ml-0 pl-0' : ''}
                        `}>
                                ...
                                block.exercises.map((exercise, eIndex) => {
                                    ...
                                    return (
                                        <div key={exercise.id} className="relative">
                                            ...
                                            {/* Exercise Insertion Point (Between items) */}
                                            {eIndex > 0 && (
                                                <div className="h-2 -mt-1 -mb-1 relative z-20 group/insert-ex flex items-center justify-center">
                                                    ...
                                                    <button 
                                                        onClick={() => openExerciseSelector(block.id, eIndex)}
                                            ...
                                            >
                                                ...
                                                <div className="flex items-start justify-between mb-4 relative z-10 flex-wrap gap-2">
                                                    <div className="flex items-center gap-3 w-full">
                                                        <div className="flex flex-col gap-1">
                                                            <button 
                                                                onClick={() => moveExercise(block.id, eIndex, eIndex - 1)} 
                                                                disabled={eIndex === 0} 
                                                                className="text-zinc-700 hover:text-white disabled:opacity-0 disabled:pointer-events-none"
                                                            >
                                                                <ChevronUp size={16} />
                                                            </button>
                                                            <button 
                                                                onClick={() => moveExercise(block.id, eIndex, eIndex + 1)} 
                                                                disabled={eIndex === block.exercises.length - 1} 
                                                                className="text-zinc-700 hover:text-white disabled:opacity-0 disabled:pointer-events-none"
                                                            >
                                                                <ChevronDown size={16} />
                                                            </button>
                                                        ...
                                                        <div className={`
                                                            w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0 cursor-grab
                                                        ...
                                                        <div className="flex-1 min-w-[150px]">
                                                            <div className="flex items-center gap-2 group/title">
                                                                <h4 className="font-bold text-white text-lg flex items-center gap-2 leading-tight">
                                                                    {exercise.name}
                                                                </h4>
                                                                <button 
                                                                    onClick={() => openExerciseEditor(exercise.exerciseId)}
                                                                    className="text-zinc-600 hover:text-[#00FF00] opacity-0 group-hover/title:opacity-100 transition-opacity"
                                                                    title={t('editor.editDetails')}
                                                                >
                                                                    <Pencil size={14} />
                                                                </button>
                                                            </div>
                                                            <input 
                                                                className="bg-transparent text-sm text-zinc-400 placeholder-zinc-700 w-full focus:outline-none focus:text-white mt-1 border-b border-transparent focus:border-zinc-700 transition-all pb-1"
                                                        ...
                                                        <div className="flex items-center gap-2 ml-auto">
                                                            <button 
                                                                onClick={() => setDeleteData({ type: 'exercise', blockId: block.id, exerciseId: exercise.id })}
                                                ...
                                                {/* Sets Table - WRAPPED FOR RESPONSIVENESS */}
                                                                            ...
                                                                            <button 
                                                                                onClick={(e) => {
                                                                                    ...
                                                                                    setOpenHeaderMenu(
                                                                                        ...
                                                                                        ? null 
                                                                                        : { blockId: block.id, exerciseId: exercise.id, colIndex }
                                                                                    );
                                                                                }}
                                                                                ...
                                                                                <div className="absolute top-full left-0 mt-1 w-32 bg-zinc-900 border border-zinc-700 rounded shadow-xl z-30 py-1">
                                                                                    {METRIC_OPTIONS.map(opt => (
                                                                                        <button
                                                                                            ...
                                                                                            onClick={(e) => {
                                                                                                e.stopPropagation();
                                                                                                changeMetricColumn(block.id, exercise.id, colIndex, opt.key);
                                                                                            }}
                                                            ...
                                                            {exercise.sets.map((set, sIndex) => (
                                                                <tr key={set.id} className="group/row hover:bg-zinc-800/30 transition-colors">
                                                                    ...
                                                                    <td className="p-2">
                                                                        <select 
                                                                            value={set.type}
                                                                            onChange={(e) => updateSet(block.id, exercise.id, set.id, 'type', e.target.value)}
                                                                    ...
                                                                    {/* Render inputs based on visibleMetrics array order */}
                                                                    {visibleMetrics.map((metricKey: any, idx) => (
                                                                        <td key={idx} className="p-2">
                                                                            <input 
                                                                                className="bg-zinc-950 border border-zinc-800 rounded px-2 py-1 w-20 text-center text-white focus:border-[#00FF00] outline-none" 
                                                ...
                                                <button 
                                                    onClick={() => addSet(block.id, exercise.id)}
